...
func TestApplyMethodByTargetName(t *testing.T) {
	// for _, s := range forceexport.SymtabNamesOfActiveFunc() {
	// 	t.Log(s)
	// }
	slice := fake.NewSlice()
	Convey("TestApplyMethodByTargetName", t, func() {

		Convey("for succ", func() {
			err := slice.Add(1)
			So(err, ShouldEqual, nil)
			patches := ApplyMethodByTargetName("github.com/henrylee2cn/gomonkey/test/fake.(*Slice).Add", func(_ *fake.Slice, _ int) error {
				return nil
			})
			defer patches.Reset()
			err = slice.Add(1)
		...
		Convey("two methods", func() {
			...
			defer patches.Reset()
			patches.ApplyMethodByTargetName("github.com/henrylee2cn/gomonkey/test/fake.(*Slice).Remove", func(_ *fake.Slice, _ int) error {
				return fake.ErrElemNotExsit
			})
			err = slice.Add(2)
			So(err, ShouldEqual, fake.ErrElemExsit)
